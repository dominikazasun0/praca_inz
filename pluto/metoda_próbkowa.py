...
Rx_0=data[0]
Rx_1=data[1] 
plt.plot(Rx_0[150:-150])
plt.plot(Rx_1[150:-150])
plt.show()
#print(Rx_0)
#print(Rx_1)
prev=Rx_0[150]
...
for i  in range(150,len(Rx_0)-150):
    if Rx_0[i].real < 0 and prev.real > 0 :
        tabela0.append(i)
        #print("było 0")
    prev=Rx_0[i]

for j  in range(150,len(Rx_1)-150):
...
with open('chanel0.txt', 'w') as plik:
    ...
    for element in Rx_0[200:-200]:
        plik.write(str(element) + '\n')

with open('chanel1.txt', 'w') as plik:
    # Zapisz dane do pliku
    for element in Rx_1[200:-200]:
        plik.write(str(element) + '\n')        
